...
exports.parseJsonToAsserts = parseJsonToAsserts;
 ...
 */
function parseJsonToAsserts(jsonString) {
    const parsed = (0, jsonc_parser_1.parse)(jsonString);
    const asserts = [];
    walkObject(parsed, '$', asserts, true);
    return `[Asserts]\n${asserts.join('\n')}`;
}
function walkObject(obj, path, asserts, isRoot = false) {
    if (obj === null) {
